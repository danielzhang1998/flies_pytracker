...
def main(videoStream, vidType, pixThreshold):
    ...
    while(cap.isOpened()):
        ...
        if (cv2.waitKey(1) & 0xFF == ord('q') or
                len(sys.argv) == 1 and datetime.now() > startTime +
                    timedelta(seconds=expDuration)
                ):
            break

        # record pixel differences in all of the ROIs
        diff = trackingTools.diffImage(
            storedFrame, currentFrame, pixThreshold, displayDiffs)
    ...
    vidInfo = trackingTools.getElapsedTime(
        vidType, vidInfo, startTime, endTime)
...
def cmdLine(videoStream, vidType, pixThreshold, append_or_new='n'):
    '''
    clearData = input(
            "(a)ppend this new data, or (n)ew experiment?   >:").rstrip()
    '''
    # this is just input() in python3
    trackingTools.keepOrAppend(append_or_new, 'dpix')
